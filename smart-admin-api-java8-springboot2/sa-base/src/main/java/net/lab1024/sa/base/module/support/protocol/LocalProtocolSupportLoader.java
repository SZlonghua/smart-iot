...
/**
 * 本地协议加载器 — 从本地 jar 或 classes 目录加载 ProtocolSupportProvider 实现。
 *
 * @Author 廖涛
 * @Date 2026/07/27
 * @Copyright 1024创新实验室
...
public class LocalProtocolSupportLoader implements ProtocolSupportLoader {
    ...
    public Mono<? extends ProtocolSupport> load(ProtocolSupportDefinition definition) {
        return Mono.fromCallable(() -> {
            log.info("[LocalLoader] 加载本地协议 — id={}, jarPath={}", definition.getId(), definition.getJarPath());
    ...
    private URLClassLoader createClassLoader(ProtocolSupportDefinition definition) throws Exception {
        ...
        return new URLClassLoader(new URL[]{url}, LocalProtocolSupportLoader.class.getClassLoader());
    }
    ...
    private void closeEntry(LoaderEntry entry) {
        log.info("[LocalLoader] 关闭协议 — id={}", entry.id);
        try {
            entry.provider.dispose();
        } catch (Exception e) {
            log.warn("[LocalLoader] provider.dispose 失败", e);
        }
        try {
            entry.classLoader.close();
        } catch (Exception e) {
            log.warn("[LocalLoader] 关闭 ClassLoader 失败", e);
        }
    }
    ...
    private ProtocolSupportProvider scanAndInstantiate(URLClassLoader classLoader, String id) throws Exception {
        ...
        for (Class<? extends ProtocolSupportProvider> clazz : subTypes) {
            log.info("[LocalLoader] 加载成功 — id={}, class={}", id, clazz.getName());
            return clazz.newInstance();
        }
        throw new IllegalStateException("未找到 ProtocolSupportProvider 实现: " + id);
    }
