...
public class MqttClientDeviceGateway extends AbstractDeviceGateway {
    ...
    public MqttClientDeviceGateway(DeviceGatewayProperties properties) { super(properties); }

    @Override protected Mono<Void> doStartup() {
        log.info("[MqttClientGateway] 启动 — id={}", properties.getId()); return Mono.empty();
    }
    @Override protected Mono<Void> doShutdown() {
        log.info("[MqttClientGateway] 关闭 — id={}", properties.getId()); return Mono.empty();
    }
}
