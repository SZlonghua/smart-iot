...
public class CoapDeviceGateway extends AbstractDeviceGateway {
    ...
    public CoapDeviceGateway(DeviceGatewayProperties properties) { super(properties.getId()); }

    @Override protected Mono<Void> doStartup() {
        log.info("[CoapGateway] 启动 — id={}", getId()); return Mono.empty();
    }
    @Override protected Mono<Void> doShutdown() {
        log.info("[CoapGateway] 关闭 — id={}", getId()); return Mono.empty();
    }
}
