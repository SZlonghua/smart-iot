...
**排除 Spring Boot 自带 Hazelcast 自动装配**（双重机制）：

1. **代码级（权威）**：`AdminApplication` 上 `@SpringBootApplication(exclude = HazelcastAutoConfiguration.class)` — 同时排除 HazelcastServerConfigConfiguration / HazelcastClientConfiguration 全部嵌套配置；IDE 可识别，运行时保证生效
2. **配置级（sa-base.yaml）**：`spring.autoconfigure.exclude: org.springframework.boot.autoconfigure.hazelcast.HazelcastAutoConfiguration` — 仅运行时生效；sa-base.yaml 由自定义 YamlProcessor（EnvironmentPostProcessor）加载，IDE 无法识别，故必须依赖代码级排除

> 若不排除：HazelcastAutoConfiguration 会用默认 Config 启动一个额外 Hazelcast 实例（multicast 加入、默认端口 5701），与自定义实例冲突。
